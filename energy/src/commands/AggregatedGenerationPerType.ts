...
export class AggregatedCommand extends Command {
  ...
  async run() {
	...
	// console.log('http://localhost:8765/energy/api/AggregatedGenerationPerType/' +`${args.AreaName}` +'/' + `${args.ProductionType}` +'/' + `${args.Resolution}` +'/date/' + splitted[0] +'-' + splitted[1] + '-' + splitted[2]);
	 ...
	 if (`${args.format}`== "undefined" || `${args.format}`=="json"){
	 const data= await axios.get('http://localhost:8765/energy/api/AggregatedGenerationPerType/' +`${args.AreaName}` +'/' + `${args.ProductionType}` +'/' + `${args.Resolution}` +'/year/' +`${args.dateinput}`);
	 console.log(data.data);
	 }
	 
	 else if (`${args.format}`=="csv") {
	 	 const data= await axios.get('http://localhost:8765/energy/api/AggregatedGenerationPerType/' +`${args.AreaName}` +'/' + `${args.ProductionType}` +'/' + `${args.Resolution}` +'/year/' + `${args.dateinput}` + "/format=" + `${args.format}`);
	 	 console.log(data.data);
	    }
