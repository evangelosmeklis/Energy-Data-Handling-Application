...
Entry.healthcheck = (result) => {
  ...
  // Create a connection to the database
	...
	connection.connect(error => {
 		 if (error) return result({message:"Unable to connect to database"});
 		 else result(null,{status: 'OK'});
	});
...
Entry.reset = (result) => { 
  sql.query(`DELETE FROM ActualTotalLoad,AggregatedGenerationPerType,AllocatedEICDetail,AreaTypeCode,DayAheadTotalLoadForecast,MapCode,ProductionType,ResolutionCode`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
  ...
  sql.query(`DELETE FROM users WHERE user<>'admin'`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Entry.cuserstatus = (username,result) => { 
  sql.query(`SELECT user,email,quota FROM users WHERE user=? `,[username] , (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Entry.findByPars14 = (AreaName,Resolution,Year,Month,result) => { //4b
  sql.query(`SELECT DISTINCT "entso-e" AS "Source","ActualVSForecastTotalLoad" AS "Dataset",A.AreaName,AreaTypeCodeText AS "AreaTypeCode",MapCodeText AS "MapCode",ResolutionCodeText AS "ResolutionCode",A.Year,A.Month,A.Day,SUM(D.TotalLoadValue) AS "DayAheadTotalLoadForecastByDayValue",SUM(A.TotalLoadValue) AS "ActualTotalLoadValue" FROM ActualTotalLoad AS A,DayAheadTotalLoadForecast AS D,AreaTypeCode AS T,MapCode AS M,ResolutionCode AS R,ProductionType AS P WHERE A.AreaName = ? AND R.ResolutionCodeText = ? AND A.Year = ? AND A.Month = ? AND A.ResolutionCodeId=R.Id AND A.MapCodeId=M.Id AND A.AreaTypeCodeId=T.Id AND A.AreaName=D.AreaName AND A.Year=D.Year AND A.Month=D.Month AND A.Day=D.Day AND A.MapCodeId=D.MapCodeId AND A.AreaTypeCodeId=D.AreaTypeCodeId AND A.ResolutionCodeId=D.ResolutionCodeId GROUP BY A.AreaName,A.AreaTypeCodeId,A.MapCodeId,A.ResolutionCodeId,A.Year,A.Month,A.Day ORDER BY A.Year,A.Month,A.Day`,[AreaName,Resolution,Year,Month] , (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Entry.findByPars15 = (AreaName,Resolution,Year,result) => { //4c
  sql.query(`SELECT DISTINCT "entso-e" AS "Source","ActualVSForecastTotalLoad" AS "Dataset",A.AreaName,AreaTypeCodeText AS "AreaTypeCode",MapCodeText AS "MapCode",ResolutionCodeText AS "ResolutionCode",A.Year,A.Month,SUM(D.TotalLoadValue) AS "DayAheadTotalLoadForecastByDayValue",SUM(A.TotalLoadValue) AS "ActualTotalLoadValue" FROM ActualTotalLoad AS A,DayAheadTotalLoadForecast AS D,AreaTypeCode AS T,MapCode AS M,ResolutionCode AS R,ProductionType AS P WHERE A.AreaName = ? AND R.ResolutionCodeText = ? AND A.Year = ? AND A.ResolutionCodeId=R.Id AND A.MapCodeId=M.Id AND A.AreaTypeCodeId=T.Id AND A.AreaName=D.AreaName AND A.Year=D.Year AND A.Month=D.Month AND A.Day=D.Day AND A.MapCodeId=D.MapCodeId AND A.AreaTypeCodeId=D.AreaTypeCodeId AND A.ResolutionCodeId=D.ResolutionCodeId GROUP BY A.AreaName,A.AreaTypeCodeId,A.MapCodeId,A.ResolutionCodeId,A.Year,A.Month ORDER BY A.Year,A.Month`,[AreaName,Resolution,Year] , (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
